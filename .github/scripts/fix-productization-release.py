...
jobs:
  prepare:
    ...
    steps:
      ...
      - name: Validate tag and prepare draft release
        id: release
        env:
  ...
  build:
    ...
    steps:
      ...
      - name: Generate SPDX SBOM
        ...
        with:
          file: dist/demonclaw-${{ needs.prepare.outputs.version }}-${{ matrix.platform }}.tar.gz
          format: spdx-json
          output-file: dist/demonclaw-${{ needs.prepare.outputs.version }}-${{ matrix.platform }}.spdx.json
      ...
      - name: Attest build provenance
        ...
        with:
          subject-path: dist/demonclaw-${{ needs.prepare.outputs.version }}-${{ matrix.platform }}.tar.gz

      - name: Attest SBOM
        uses: actions/attest@1e69f48acb82d1966a394da916b4c1698aa569d6 # v4.2.2
        with:
          subject-path: dist/demonclaw-${{ needs.prepare.outputs.version }}-${{ matrix.platform }}.tar.gz
          sbom-path: dist/demonclaw-${{ needs.prepare.outputs.version }}-${{ matrix.platform }}.spdx.json
...
Path(".github/workflows/release.yml").write_text(release, encoding="utf-8")

readme_path = Path("README.md")
readme = readme_path.read_text(encoding="utf-8")
needle = "Download the release archive for `linux-x86_64` or `linux-aarch64`, verify its adjacent SHA-256 file, extract it, then run:\n"
replacement = "Download the release archive for `linux-x86_64` or `linux-aarch64`, verify its adjacent SHA-256 file and GitHub provenance, extract it, then run:\n"
if needle not in readme:
    raise SystemExit("README installation text anchor missing")
...
if install_block not in readme:
    raise SystemExit("README installer block anchor missing")
readme = readme.replace(install_block, attest_block, 1)
readme_path.write_text(readme, encoding="utf-8")

checklist_path = Path("RELEASE_CHECKLIST.md")
checklist = checklist_path.read_text(encoding="utf-8")
needle = "- [ ] verify each published SHA-256 checksum\n"
replacement = "- [ ] verify each published SHA-256 checksum\n- [ ] verify both SPDX SBOM assets are present\n- [ ] verify both release tarballs with `gh attestation verify ... -R BlueDot-IT/DemonClaw`\n"
if needle not in checklist:
    raise SystemExit("release checklist checksum anchor missing")
checklist_path.write_text(checklist.replace(needle, replacement, 1), encoding="utf-8")
